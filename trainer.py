...
# 自定义渲染器 only coarse
from volumetric_rendering.myrenderfun import ImportanceRenderer
# 改进渲染器 coarse to fine
#from volumetric_rendering.render_corase2fine import ImportanceRenderer
...
def train(audio_path, img_path, model_path, save_path, audio_feature_path=None, epoch=160, mode="train"):
    ...
    if use_sync_loss:
        ...
        load_checkpoint(syncnet_checkpoint_path, syncnet, None, reset_optimizer=False, overwrite_global_states=False)
    ...
    if mode == 'inference':
        ...
        log_dir = save_path
        if not os.path.exists(os.path.join(log_dir, "addfine")):
            os.makedirs(os.path.join(log_dir, "addfine"))
        image_name = os.path.basename(img_path)[:-4]+ "_" + os.path.basename(audio_path)[:-4] + ".mp4"

        video_path = os.path.join(log_dir, "addfine", image_name)
...
def percetual_loss(cur_frames, gt, norm=False):
    url = 'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada-pytorch/pretrained/metrics/vgg16.pt'
    with dnnlib.util.open_url(url) as f:
        vgg16 = torch.jit.load(f).eval().to(cur_frames.device)   
    target_features = vgg16(cur_frames, resize_images=False, return_lpips=True)
    synth_features = vgg16(gt, resize_images=False, return_lpips=True)
    dist = (target_features - synth_features).square().sum()*100
    if norm:
        (dist - dist.min()) / (dist.max() - dist.min())
    print("percetual_loss", dist)
    return dist 
# def calulate_loss(cur_frames, gt, loss_type = "all"):
#     if loss_type == "percetual+mse":
#         loss = 0
#         mseloss = torch.nn.MSELoss(reduction='sum') 
#         loss += torch.sqrt(mseloss(cur_frames.detach(), gt.detach()))
#         loss += percetual_loss(cur_frames, gt)
#     else:
#         loss = torch.nn.MSELoss(reduction='sum') 
#     return loss 


def calulate_loss(cur_frames, gt, audio_seg = None, loss_type = "percetual+mse"):
    if loss_type == "percetual+mse":
        loss = 0
        #loss += torch.sqrt(img2mse(cur_frames, gt))
        loss += l2loss(cur_frames, gt)
        loss += percetual_loss(cur_frames, gt)
    if loss_type == "percetual+mse+synclip":
        loss = 0
        #loss += torch.sqrt(img2mse(cur_frames, gt))
        loss += l2loss(cur_frames, gt)
        loss += percetual_loss(cur_frames, gt)
...
def l2loss(x,y):return (((x - y) ** 2 + 1e-6) ** 0.5).mean()
...
def lip_loss(mel, g, model):
    ...
    # B, 3 * T, H//2, W
    a, v = model(mel, g)
    y = torch.ones(g.size(0), 1).float().to(model.device)
...
def load_checkpoint(path, model, optimizer, reset_optimizer=False, overwrite_global_states=True):
    """load syncnet parameter for loss"""
    ...
    if not reset_optimizer:
        ...
        if optimizer_state is not None:
            print("load {} optimizer state from {}".format(model, path))
