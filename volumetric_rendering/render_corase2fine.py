...
def sample_from_featuremap(plane_axes, feature_map, coordinates, mode='bilinear', padding_mode='zeros', box_warp=1.0):
    ...
    projected_coordinates = project_onto_planes(plane_axes, coordinates).unsqueeze(1).to(feature_map.device)
    #print("projected_coordinates",projected_coordinates)
    #print("coordinates.shape",coordinates.shape) # coordinates.shape torch.Size([1, 196608, 3]) 
    #print("projected_coordinates shape",projected_coordinates.shape) # projected_coordinates shape torch.Size([1, 1, 196608, 2]) 
    ...
    #print("output_features.shape",output_features.shape) #output_features.shape torch.Size([1, 1, 196608, 64])
    return output_features
...
class ImportanceRenderer(torch.nn.Module):
    ...
    def gen_ray(self, render_resolusion, K,):
        with torch.no_grad():
            cam_rays_d = get_camera_rays(render_resolusion, render_resolusion, K)
    ...
    def forward(self, feature_map, decoder, ray_origins, ray_directions, rendering_options):
        ...
        ray_origins, ray_directions = self.gen_ray(self.render_resolusion,self.K)#[4096, 3])
        ray_origins = ray_origins.unsqueeze(0).to(feature_map.device)
        ray_directions = ray_directions.unsqueeze(0).to(feature_map.device)
        self.plane_axes = self.plane_axes.to(feature_map.device)
        
        #print("ray_origins.shape",ray_origins.shape) #  torch.Size([1, 4096, 3]) 
        ...
        if self.rendering_options['ray_start'] == self.rendering_options['ray_end'] == 'auto':
            ...
            ray_start = torch.tensor(0).expand(ray_start.shape)
            ray_end = torch.tensor(1).expand(ray_end.shape)
            
            # print("ray_start.shape, ray_end.shape",ray_start.shape, ray_end.shape) #
            # print("ray_start.shape, ray_end.shape",ray_start, ray_end)
        ...
        depths_coarse = depths_coarse.to(feature_map.device)
        #print("depths_coarse.shape",depths_coarse.shape) #torch.Size([1, 4096, 48, 1])
        batch_size, num_rays, samples_per_ray, _ = depths_coarse.shape    
        #print("ray_origins.shape",ray_origins.shape) # ([1, 4096, 3]) 
        #print("depths_coarse.shape",depths_coarse.shape) #torch.Size([1, 4096, 48, 1]) 
        # 这里是得到的采样点  
        sample_coordinates = (ray_origins.unsqueeze(-2) + depths_coarse * ray_directions.unsqueeze(-2)).reshape(batch_size, -1, 3)
        # print("max:",sample_coordinates.max())
        # print("min:",sample_coordinates.min())
        # print(sample_coordinates.shape)
        min_x = sample_coordinates[0,:,0].min()
        max_x = sample_coordinates[0,:,0].max()
        min_y = sample_coordinates[0,:,1].min()
        max_y = sample_coordinates[0,:,1].max()
        sample_coordinates[0,:,0] = 2 * (sample_coordinates[0,:,0] - min_x) / (max_x - min_x) - 1
        sample_coordinates[0,:,1] = 2 * (sample_coordinates[0,:,1] - min_y) / (max_y - min_y) - 1
        # print("max:",sample_coordinates.max())
        # print("min:",sample_coordinates.min())
