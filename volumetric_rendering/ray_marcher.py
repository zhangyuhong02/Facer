...
class MipRayMarcher2(nn.Module):
    ...
    def run_forward(self, colors, densities, depths, rendering_options):
        ...
        densities_mid = densities_mid.to(colors.device)
        deltas = deltas.to(colors.device)
        colors_mid = colors_mid.to(colors.device)
        depths_mid = depths_mid.to(colors.device)
